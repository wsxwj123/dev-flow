...
3. **角色隔离**：方案、审查、测试设计、开发、模块测试、裁判、安全审计等都是独立子代理（调研代理是方案阶段的前置独立代理，另计）。子代理看不到会话历史，只给它文件路径和任务描述，不要在 prompt 里透露方案/代码出自谁手。审查/裁判/安全审计时都说成"隔壁同事做的"。多轮循环按"**作者续用，判官新开**"：方案修订、打回重修可按名字续用原代理（代理自己的上下文不算会话历史），审查/测试设计/裁判/模块测试/安全审计每轮必须新开；跨角色唤醒一律禁止；续话消息开头要原样重贴本阶段的逐字禁令。细则见 PLATFORMS.md"续用子代理"节。
...
5. **中途变大立刻升档**：发现这次改动要新增行为路径（同一处小改在几个文件里重复出现不算，那仍是微任务，和上面"分档看行为"同一把尺）、要加分支绕过、根因不在这一处、或者碰到了用户数据/密钥/外部网络，当场停下重新分级走流程，不许在微任务档里把它做完。升档后从 02 方案进（小任务档合并卡点照旧），已改一半的先恢复原状（撤销或 stash），别带着半成品进流程。微任务判错的代价是这次改动全程无人复核，判错方向只能往严了走。
...
- ❌ 续用判定型代理（审查/测试设计/裁判/模块测试/安全审计每轮必须新开）；把作者代理唤去当判官；把代码地图发给盲审、裁判或测试设计代理
