...
打回重修、方案修订这类同角色多轮场景，优先续用原代理：它的上下文还在，读过的东西都是缓存前缀，省一整轮重读。只许同角色续用；跨角色唤醒（方案代理去当审查、开发代理去当裁判）破坏角色隔离，一律禁止。盲审、测试设计、首轮裁判、安全审计必须新开，失忆是它们的产品属性不是浪费。

| | Claude Code | OpenCode | Codex |
|---|---|---|---|
| 续话机制 | 派时命名，`SendMessage({to: 名字})` 续话，同一主会话内有效 | 未核实（child session 理论上可续） | 未核实（thread 理论上可 resume） |
